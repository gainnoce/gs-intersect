...
import { DiffProportionsInputPanel }   from "@/components/DiffProportionsInputPanel";
import { DiffProportionsChart }        from "@/components/DiffProportionsChart";
import { DiffProportionsOptimalCard }  from "@/components/DiffProportionsOptimalCard";
import { DiffProportionsResultsTable } from "@/components/DiffProportionsResultsTable";
import { RawOutput }                   from "@/components/RawOutput";
import { LoadingProgress }             from "@/components/LoadingProgress";
import { runDiffProportions }          from "@/lib/api";
import type { DiffProportionsInputs, DiffProportionsResponse } from "@/lib/api";
import { AlertCircle } from "lucide-react";

export default function DiffProportionsPage() {
  const [result,     setResult]     = useState<DiffProportionsResponse | null>(null);
  const [lastInputs, setLastInputs] = useState<DiffProportionsInputs | null>(null);
  const [loading,    setLoading]    = useState(false);
  const [phase,      setPhase]      = useState<"connecting" | "computing" | "processing">("connecting");
  const [error,      setError]      = useState<string | null>(null);

  const handleRun = async (inputs: DiffProportionsInputs) => {
    setLoading(true);
    setPhase("connecting");
    ...
    try {
      const data = await runDiffProportions(inputs);
      resolved = true;
  ...
  const nCount = lastInputs ? lastInputs.n_max - lastInputs.n_min + 1 : 181;
    ...
    <div className="min-h-screen flex flex-col">
      <main className="flex-1 max-w-screen-2xl mx-auto w-full px-6 py-6">
        <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-6 items-start">
          ...
          <DiffProportionsInputPanel onRun={handleRun} loading={loading} />
            ...
            {!result && !loading && !error && (
              <div className="flex flex-col items-center justify-center h-96 rounded-xl border-2 border-dashed border-az-light-platinum text-center px-8 bg-white print-hidden">
                ...
                <p className="text-az-platinum text-xs mt-1.5 max-w-xs leading-relaxed">
                  ...
                  to find the utility-maximising parallel 2-arm design.
                </p>
              </div>
            )}
